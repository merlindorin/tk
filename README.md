...
`tk init` and `tk update` never rewrite your whole `Taskfile.yaml` or `.envrc`: they only touch the lines they
generated, which are marked with a trailing `#!tk` comment.
...
(via `excludes` in `.tk.yaml`) drops its marked include and nothing more. Includes generated by older versions of `tk`
